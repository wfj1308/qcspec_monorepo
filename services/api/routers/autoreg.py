...
def get_supabase() -> Client:
    url = (os.getenv("GITPEG_SUPABASE_URL") or os.getenv("SUPABASE_URL") or "").strip()
    key = (
        os.getenv("GITPEG_SUPABASE_SERVICE_ROLE_KEY")
        or os.getenv("SUPABASE_SERVICE_ROLE_KEY")
        or os.getenv("SUPABASE_SERVICE_KEY")
        or ""
    ).strip()
    if not url or not key:
        raise HTTPException(500, "supabase not configured for autoreg")
...
def _normalize_request(req: AutoRegisterProjectRequest) -> dict[str, Any]:
...
def _upsert_autoreg(sb: Client, payload: dict[str, Any]) -> dict[str, Any]:
    now_iso = _now_iso()
...
async def autoreg_project(req: AutoRegisterProjectRequest, sb: Client = Depends(get_supabase)):
    payload = _normalize_request(req)
    try:
        upsert_result = _upsert_autoreg(sb, payload)
...
async def autoreg_project_alias(req: AutoRegisterProjectRequest, sb: Client = Depends(get_supabase)):
    return await autoreg_project(req, sb)
