...
from services.api.domain.boq.audit_engine import get_item_sovereign_history, run_boq_audit_engine
from services.api.domain.boq.support_flows import (
...
def _get_item_history_safe(*, project_uri: str, subitem_code: str, sb: Any) -> dict[str, Any]:
    try:
...
def _scoped_rows(entries: list[Any], *, item_no: str, item_uri: str) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
...
def _get_evidence_center_evidence_smu(*, project_uri: str, smu_id: str, sb: Any) -> dict[str, Any]:
    realtime = get_boq_realtime_status(sb=sb, project_uri=project_uri, limit=10000)
    items = [
        _as_dict(x)
        for x in _as_list(realtime.get("items"))
        if _to_text(_as_dict(x).get("item_no") or "").strip().startswith(smu_id)
    ]
    if not items:
        raise HTTPException(404, f"no BOQ items found under smu_id={smu_id}")
    ...
    for row in items:
        ...
        if not item_no or not item_uri:
            continue
        history = _get_item_history_safe(project_uri=project_uri, subitem_code=item_no, sb=sb)

        for one in _scoped_rows(_as_list(history.get("timeline")), item_no=item_no, item_uri=item_uri):
            timeline.append(one)
            pid = _to_text(one.get("proof_id") or "").strip()
            if pid:
                latest_proof_id = pid

        for one in _scoped_rows(_as_list(history.get("documents")), item_no=item_no, item_uri=item_uri):
        ...
        evidence.extend(_scoped_rows(ev_list, item_no=item_no, item_uri=item_uri))
        scan_entries.extend(_scoped_rows(_as_list(ev_payload.get("scan_entries")), item_no=item_no, item_uri=item_uri))
        meshpeg_entries.extend(_scoped_rows(_as_list(ev_payload.get("meshpeg_entries")), item_no=item_no, item_uri=item_uri))
        formula_entries.extend(_scoped_rows(_as_list(ev_payload.get("formula_entries")), item_no=item_no, item_uri=item_uri))
        gateway_entries.extend(_scoped_rows(_as_list(ev_payload.get("gateway_entries")), item_no=item_no, item_uri=item_uri))
    ...
    if trace_item_uri:
        try:
            ...
            asset_origin = {}
    smu_statement = (
        f"SMU {smu_id} contract_total {round(float(ledger.get('contract_quantity') or 0.0), 4)}, "
    ...
    if total_proof_hash:
        try:
            sealing_trip = build_sealing_trip(
                total_proof_hash=total_proof_hash,
                project_uri=project_uri,
                boq_item_uri=f"{project_uri.rstrip('/')}/smu/{smu_id}",
                smu_id=smu_id,
            )
        except Exception:
            sealing_trip = {}
...
def _get_evidence_center_evidence_item(
    *,
    ...
    if not item_uri:
        ...
        history = _get_item_history_safe(project_uri=p_uri, subitem_code=code, sb=sb)
...
def get_evidence_center_evidence(
    *,
    ...
    if scope_smu_id:
        ...
        return _get_evidence_center_evidence_smu(project_uri=p_uri, smu_id=scope_smu_id, sb=sb)
    return _get_evidence_center_evidence_item(
        project_uri=p_uri,
        subitem_code=subitem_code,
        boq_item_uri=boq_item_uri,
        sb=sb,
...
def get_boq_realtime_status_flow(*, project_uri: str, sb: Any) -> dict[str, Any]:
