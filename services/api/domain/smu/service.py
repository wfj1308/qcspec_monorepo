...
from services.api.domain.smu.helpers import (
...
class SMUService(BaseService):
    async def import_genesis(self, **kwargs: Any) -> Any:
        ...
        return await self.run_guarded("smu_import_genesis", run_in_threadpool, smu_genesis_import_flow, **kwargs, sb=supabase)

    async def preview_genesis(self, **kwargs: Any) -> Any:
        supabase = self.require_supabase()
        return await self.run_guarded("smu_preview_genesis", run_in_threadpool, smu_genesis_preview_flow, **kwargs, sb=supabase)
    ...
    async def get_node_context(self, **kwargs: Any) -> Any:
        supabase = self.require_supabase()
        return await self.run_guarded("smu_node_context", smu_node_context_flow, sb=supabase, **kwargs)

    async def execute_trip(self, *, body: Any) -> Any:
        supabase = self.require_supabase()
        return await self.run_guarded("smu_execute", smu_execute_flow, body=body, sb=supabase)

    async def sign_trip(self, *, body: Any) -> Any:
        supabase = self.require_supabase()
        return await self.run_guarded("smu_sign", smu_sign_flow, body=body, sb=supabase)

    async def validate_logic(self, *, body: Any) -> Any:
        supabase = self.require_supabase()
        return await self.run_guarded("smu_validate_logic", smu_validate_logic_flow, body=body, sb=supabase)

    async def freeze_asset(self, *, body: Any) -> Any:
        supabase = self.require_supabase()
        return await self.run_guarded("smu_freeze", smu_freeze_flow, body=body, sb=supabase)

    async def retry_erpnext_push(self, *, limit: int) -> Any:
        supabase = self.require_supabase()
        return await self.run_guarded("smu_retry_erpnext_push", retry_erpnext_push_queue, sb=supabase, limit=limit)
