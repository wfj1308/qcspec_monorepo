...
class ExecutionService(BaseService):
    ...
    async def execute_triprole(self, *, body: Any) -> Any:
        return await self.run_guarded("execute_triprole", execute_triprole_action_flow, body=body, sb=self.require_supabase())

    async def ingest_sensor_data(self, *, body: Any) -> Any:
        return await self.run_guarded("ingest_sensor_data", ingest_sensor_data_flow, body=body, sb=self.require_supabase())

    async def record_lab_test(self, *, body: Any) -> Any:
        return await self.run_guarded("record_lab_test", record_lab_test_flow, body=body, sb=self.require_supabase())

    async def calc_frequency(self, *, body: Any) -> Any:
        return await self.run_guarded("calc_frequency", calc_inspection_frequency_flow, body=body, sb=self.require_supabase())
    ...
    async def get_provenance(self, *, utxo_id: str) -> Any:
        return await self.run_guarded("get_provenance", aggregate_provenance_chain_flow, utxo_id=utxo_id, sb=self.require_supabase())

    async def get_full_lineage(self, *, utxo_id: str) -> Any:
        return await self.run_guarded("get_full_lineage", get_full_lineage_flow, utxo_id=utxo_id, sb=self.require_supabase())
    ...
    async def transfer_asset(self, *, body: Any) -> Any:
        return await self.run_guarded("transfer_asset", transfer_asset_flow, body=body, sb=self.require_supabase())

    async def apply_variation(self, *, body: Any) -> Any:
        return await self.run_guarded("apply_variation", apply_variation_flow, body=body, sb=self.require_supabase())

    async def replay_offline_packets(self, *, body: Any) -> Any:
        return await self.run_guarded("replay_offline_packets", replay_offline_packets_flow, body=body, sb=self.require_supabase())

    async def scan_confirm_signature(self, *, body: Any) -> Any:
        return await self.run_guarded("scan_confirm_signature", scan_confirm_signature_flow, body=body, sb=self.require_supabase())

    async def open_remediation(self, *, body: Any) -> Any:
        return await self.run_guarded("open_remediation", open_remediation_trip_flow, body=body, sb=self.require_supabase())

    async def remediation_reinspect(self, *, body: Any) -> Any:
        return await self.run_guarded("remediation_reinspect", remediation_reinspect_flow, body=body, sb=self.require_supabase())

    async def close_remediation(self, *, body: Any) -> Any:
        return await self.run_guarded("close_remediation", close_remediation_trip_flow, body=body, sb=self.require_supabase())
